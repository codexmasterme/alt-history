...
score_original = """        if (option.scores) {
            for (let k in option.scores) {
                this.scores[k] += option.scores[k];
            }
        }"""
score_replacement = """        if (option.scores) {
            let maxScore = 0;
            for (let k in option.scores) {
                this.scores[k] += option.scores[k];
...
bind_original = """document.getElementById('restart-btn').addEventListener('click', () => {"""
bind_replacement = """document.getElementById('rewind-btn').addEventListener('click', () => {
...
ending_original = """        document.getElementById('ending-screen').classList.remove('hidden');"""
ending_replacement = """        document.getElementById('ending-screen').classList.remove('hidden');
...
restart_original = """this.scores = {"""
restart_replacement = """this.history = [];
            this.lastDominantTraits = [];
            this.comboCount = 0;
            this.scores = {"""
...
methods = """
    showComboToast(traits, count) {
        ...
        document.getElementById('combo-desc').textContent = `【${name}】`;
        document.getElementById('combo-multiplier').textContent = `x${count}`;
...
print("Patch applied to main.js successfully!")
